...
def main() -> None:
    ...
    if dataset != "gsm8k_semantic_distractor":
        raise ValueError(f"Unsupported dataset for benchmark_generation: {dataset}")

    from pipeline.benchmark_generation.gsm8k_semantic_distractor.eval import (
        run_generation_pipeline,
    )

    raw_results, processed_results = run_generation_pipeline(config)
